...
#ifndef _HARDCAMERA_DESCRIPTIONS_HPP_
#define _HARDCAMERA_DESCRIPTIONS_HPP_
...
/// Functor which receives a AnyHardCamera and return a copy of its
/// HardCamera base class.
struct GetHardCamera
  :public boost::static_visitor<HardCamera>
{
  template <class T>
  const HardCamera operator()(const T& config) const
  {
    return config;
  }

  template <class T>
  HardCamera operator()(T& config)
  {
    return config;
  }
};
...
#endif /* _HARDCAMERA_DESCRIPTIONS_HPP_ */
