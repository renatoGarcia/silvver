...
dc1394_bayer_NearestNeighbor(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx, int sy, int tile)
{
    ...
    int width = sx;
    int height = sy;
    int blue = tile == LIBDC1394_COLOR_FILTER_BGGR
        || tile == LIBDC1394_COLOR_FILTER_GBRG ? -1 : 1;
    int start_with_green = tile == LIBDC1394_COLOR_FILTER_GBRG
        || tile == LIBDC1394_COLOR_FILTER_GRBG;
    int i, imax, iinc;

    if ((tile>LIBDC1394_COLOR_FILTER_MAX)||(tile<LIBDC1394_COLOR_FILTER_MIN))
      return LIBDC1394_INVALID_COLOR_FILTER;
...
dc1394_bayer_Downsample(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx, int sy, int tile)
{
    ...
    switch (tile) {
    case LIBDC1394_COLOR_FILTER_GRBG:
    case LIBDC1394_COLOR_FILTER_BGGR:
        outR = &rgb[0];
        outG = &rgb[1];
        outB = &rgb[2];
        break;
    case LIBDC1394_COLOR_FILTER_GBRG:
    case LIBDC1394_COLOR_FILTER_RGGB:
        outR = &rgb[2];
        outG = &rgb[1];
        outB = &rgb[0];
        break;
...
dc1394_bayer_VNG(const uint8_t *restrict bayer,
                 ...
                 libdc1394color_filter_t pattern)
{
...
dc1394_bayer_VNG_uint16(const uint16_t *restrict bayer,
                        ...
                        libdc1394color_filter_t pattern, int bits)
{
    const int height = sy, width = sx;
    static const signed char *cp;
    /* the following has the same type as the image */
...
static libdc1394bool_t ahd_inited = LIBDC1394_FALSE; /* WARNING: not multi-processor safe */
...
libdc1394error_t
dc1394_bayer_decoding_8bit(const uint8_t *restrict bayer, uint8_t *restrict rgb, uint32_t sx, uint32_t sy, libdc1394color_filter_t tile, dc1394bayer_method_t method)
{
    switch (method) {
    case LIBDC1394_BAYER_METHOD_NEAREST:
        return dc1394_bayer_NearestNeighbor(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_SIMPLE:
        return dc1394_bayer_Simple(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_BILINEAR:
        return dc1394_bayer_Bilinear(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_HQLINEAR:
        return dc1394_bayer_HQLinear(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_DOWNSAMPLE:
        return dc1394_bayer_Downsample(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_EDGESENSE:
        return dc1394_bayer_EdgeSense(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_VNG:
        return dc1394_bayer_VNG(bayer, rgb, sx, sy, tile);
    case LIBDC1394_BAYER_METHOD_AHD:
        return dc1394_bayer_AHD(bayer, rgb, sx, sy, tile);
    default:
        return LIBDC1394_INVALID_BAYER_METHOD;
  }

}

libdc1394error_t
dc1394_bayer_decoding_16bit(const uint16_t *restrict bayer, uint16_t *restrict rgb, uint32_t sx, uint32_t sy, libdc1394color_filter_t tile, dc1394bayer_method_t method, uint32_t bits)
{
    switch (method) {
    case LIBDC1394_BAYER_METHOD_NEAREST:
