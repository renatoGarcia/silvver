...
#ifndef _IPL_IMAGE_WRAPPER_HPP_
#define _IPL_IMAGE_WRAPPER_HPP_
  ...
  :iplImage(NULL)
{
  if (iplImageSrc)
  {
    this->iplImage = cvCloneImage(iplImageSrc);
  }
}
...
IplImageWrapper::operator=(const IplImageWrapper& iplImageWrapper)
{
  ...
  this->iplImage =
    iplImageWrapper.iplImage ? cvCloneImage(iplImageWrapper.iplImage) : NULL;
...
IplImageWrapper::loadImage(const std::string& filename, int iscolor)
{
  ...
  return this->iplImage ? true : false;
}
